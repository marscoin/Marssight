...
var defaultLanguage = localStorage.getItem("insight-language") || "en",
    defaultCurrency = localStorage.getItem("insight-currency") || "MARS";
    ...
    .controller("CurrencyController", function($scope, $rootScope, Currency) {
        ...
        $rootScope.currency.getConvertion = function(value) {
            if (value = 1 * value, !isNaN(value) && "undefined" != typeof value && null !== value) {
                ...
                var response;
                return "USD" === this.symbol ? response = _roundFloat(value * this.factor, 2) : "mMARS" === this.symbol ? (this.factor = 1e3, response = _roundFloat(value * this.factor, 5)) : "uMARS" === this.symbol ? (this.factor = 1e6, response = _roundFloat(value * this.factor, 2)) : (this.factor = 1, response = value), 1e-7 > response && (response = response.toFixed(8)), response + " " + this.symbol
            }
            return "value error"
            ...
            }) : $rootScope.currency.factor = "mMARS" === currency ? 1e3 : "uMARS" === currency ? 1e6 : 1
        ...
        var pageNum = 0,
            ...
            _aggregateItems = function(items) {
                ...
                for (var l = items.length, ret = [], tmp = {}, u = 0, i = 0; l > i; i++) {
                    var notAddr = !1;
                    if (items[i].scriptSig && !items[i].addr && (items[i].addr = "Unparsed address [" + u++ +"]", items[i].notAddr = !0, notAddr = !0), items[i].scriptPubKey && !items[i].scriptPubKey.addresses && (items[i].scriptPubKey.addresses = ["Embedded Data"], items[i].notAddr = !0, notAddr = !0, items[i].OPRETURN = _dectag(_hexToAscii(String(items[i].scriptPubKey.asm).split("OP_RETURN ")[1])), items[i].OPRETURN_IPFSLINK = _ipfsfy(_hexToAscii(String(items[i].scriptPubKey.asm).split("OP_RETURN ")[1]))), items[i].scriptPubKey && items[i].scriptPubKey.addresses.length > 1) items[i].addr = items[i].scriptPubKey.addresses.join(","), ret.push(items[i]);
                    else {
