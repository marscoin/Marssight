...
Script.prototype.parseY = function() {
  this.chunks = [];

  var parser = new Parser(this.buffer);
  log.info('Starting script parse:', {
  ...
  while (!parser.eof()) {
    ...
    //log.info('Parsing opcode:', {
    //  opcode: opcode,
    //  opcodeHex: opcode.toString(16),
    //  opcodeMap: Opcode.reverseMap[opcode]
    //});

    var len, chunk;
    if (opcode > 0 && opcode < Opcode.map.OP_PUSHDATA1) {
      // Read some bytes of data, opcode value is the length of data
      chunk = parser.buffer(opcode);
      //log.info('Push data:', {
      //  length: opcode,
  ...
  log.info('Finished parsing, chunks:', {
    chunkCount: this.chunks.length,
    chunks: this.chunks.map(function(c) {
      return Buffer.isBuffer(c) ? c.toString('hex') : Opcode.reverseMap[c];
    })
  });
};
